...
namespace
{
	...
	emulator_object<KUSER_SHARED_DATA64> setup_kusd(x64_emulator& emu, bool use_relative_time)
	{
		// TODO: Fix that. Use hooks to feed dynamic data, e.g. time values

		emu.allocate_memory(KUSD_ADDRESS, page_align_up(sizeof(KUSER_SHARED_DATA64)), memory_permission::read);

		const emulator_object<KUSER_SHARED_DATA64> kusd_object{emu, KUSD_ADDRESS};
		kusd_object.access([&](KUSER_SHARED_DATA64& kusd)
		{
	...
	using exception_record_map = std::unordered_map<const EXCEPTION_RECORD<EmulatorTraits<Emu64>>*, emulator_object<EXCEPTION_RECORD<EmulatorTraits<Emu64>>>>;

	emulator_object<EXCEPTION_RECORD<EmulatorTraits<Emu64>>> save_exception_record(emulator_allocator& allocator,
	                                                        const EXCEPTION_RECORD<EmulatorTraits<Emu64>>& record,
	                                                        exception_record_map& record_mapping)
	{
		const auto record_obj = allocator.reserve<EXCEPTION_RECORD<EmulatorTraits<Emu64>>>();
		...
		if (record.ExceptionRecord)
		{
			...
			emulator_object<EXCEPTION_RECORD<EmulatorTraits<Emu64>>> nested_record_obj{allocator.get_emulator()};
			const auto nested_record = record_mapping.find(reinterpret_cast<EXCEPTION_RECORD<EmulatorTraits<Emu64>>*>(record.ExceptionRecord));
			...
			{
				nested_record_obj = save_exception_record(allocator, *reinterpret_cast<EXCEPTION_RECORD<EmulatorTraits<Emu64>>*>(record.ExceptionRecord),
				                                          record_mapping);
			}

			record_obj.access([&](EXCEPTION_RECORD<EmulatorTraits<Emu64>>& r)
			{
				r.ExceptionRecord = reinterpret_cast<unsigned long>(nested_record_obj.ptr());
			});
		}
	...
	size_t calculate_exception_record_size(const EXCEPTION_RECORD<EmulatorTraits<Emu64>>& record)
	{
		std::unordered_set<const EXCEPTION_RECORD<EmulatorTraits<Emu64>>*> records{};
		size_t total_size = 0;

		const EXCEPTION_RECORD<EmulatorTraits<Emu64>>* current_record = &record;
		while (current_record)
		{
			...
			current_record = reinterpret_cast<EXCEPTION_RECORD<EmulatorTraits<Emu64>>*>(record.ExceptionRecord);
		}
	...
	void dispatch_exception_pointers(x64_emulator& emu, const uint64_t dispatcher, const EXCEPTION_POINTERS<EmulatorTraits<Emu64>> pointers)
	{
		constexpr auto mach_frame_size = 0x40;
		constexpr auto context_record_size = 0x4F0;
		const auto exception_record_size = calculate_exception_record_size(*reinterpret_cast<EXCEPTION_RECORD<EmulatorTraits<Emu64>>*>(pointers.ExceptionRecord));
		...
		const auto exception_record_obj = save_exception_record(allocator, *reinterpret_cast<EXCEPTION_RECORD<EmulatorTraits<Emu64>>*>(pointers.ExceptionRecord));
	                               ...
	                               const memory_operation operation)
	{
		...
		EXCEPTION_RECORD<EmulatorTraits<Emu64>> record{};
		memset(&record, 0, sizeof(record));
		record.ExceptionCode = static_cast<DWORD>(STATUS_ACCESS_VIOLATION);
		record.ExceptionFlags = 0;
		record.ExceptionRecord = 0;
		record.ExceptionAddress = reinterpret_cast<unsigned long>(emu.read_instruction_pointer());
		record.NumberParameters = 2;
		record.ExceptionInformation[0] = map_violation_operation_to_parameter(operation);
		record.ExceptionInformation[1] = address;

		EXCEPTION_POINTERS<EmulatorTraits<Emu64>> pointers{};
		pointers.ContextRecord = reinterpret_cast<unsigned long>(&ctx);
		pointers.ExceptionRecord = reinterpret_cast<unsigned long>(&record);
	  ...
	  , last_registers(context.default_register_set)
{
	...
	{
		teb_obj.ClientId.UniqueProcess = reinterpret_cast<DWORD64>(1ul);
		teb_obj.ClientId.UniqueThread = reinterpret_cast<DWORD64>(static_cast<uint64_t>(this->id));
