...
namespace
{
	                                          ...
	                                          const emulator_object<LARGE_INTEGER> performance_frequency)
	{
		try
		{
			...
			{
				int64_t frequency{};
				c.proc.kusd.access([&](const KUSER_SHARED_DATA64& kusd)
				{
	                          ...
	                          const emulator_object<OBJECT_ATTRIBUTES<EmulatorTraits<Emu64>>> object_attributes)
	{
		...
		if (attributes.RootDirectory)
		{
			...
			std::filesystem::path full_path = parent_handle->hive / parent_handle->path / key_path;
	                              ...
	                              const EVENT_TYPE event_type, const BOOLEAN initial_state)
	{
		...
		if (object_attributes)
		{
			...
			{
				name = read_unicode_string(c.emu, reinterpret_cast<UNICODE_STRING<EmulatorTraits<Emu64>>*>(attributes.ObjectName));
			}
		}
	                                   ...
	                                   const ULONG /*win32_protect*/)
	{
		...
		if (section_handle == SHARED_SECTION)
		{
			...
			c.proc.kusd.access([&](const KUSER_SHARED_DATA64& kusd)
			{
				const std::u16string_view windows_dir = kusd.NtSystemRoot.arr;
			...
			{
				const auto dir_address = c.proc.kusd.value() + offsetof(KUSER_SHARED_DATA64, NtSystemRoot);

				ucs.Buffer = reinterpret_cast<unsigned long>(dir_address - obj_address);
			...
			sysdir_obj.access([&](UNICODE_STRING<EmulatorTraits<Emu64>>& ucs)
			{
				...
				ucs.Buffer = reinterpret_cast<unsigned long>(reinterpret_cast<uint64_t>(ucs.Buffer) - obj_address);
			});
	                                     ...
	                                     const emulator_object<uint32_t> return_length)
	{
		...
		if (info_class == MemoryBasicInformation)
		{
			if (return_length)
			{
				return_length.write(sizeof(MEMORY_BASIC_INFORMATION64));
			}

			if (memory_information_length != sizeof(MEMORY_BASIC_INFORMATION64))
			{
				return STATUS_BUFFER_OVERFLOW;
			}

			const emulator_object<MEMORY_BASIC_INFORMATION64> info{c.emu, memory_information};

			info.access([&](MEMORY_BASIC_INFORMATION64& image_info)
			{
	                                          ...
	                                          const emulator_object<uint32_t> return_length)
	{
		...
		if (info_class == ProcessImageFileNameWin32)
		{
			...
			info.access([&](UNICODE_STRING<EmulatorTraits<Emu64>>& str)
			{
				...
				str.Buffer = reinterpret_cast<unsigned long>(buffer_start);
			});
	                              ...
	                              const emulator_object<ULONG> connection_info_length)
	{
		...
		client_shared_memory.access([&](PORT_VIEW64& view)
		{
			...
			view.ViewBase = reinterpret_cast<unsigned long>(p.view_base);
	                                      ...
	                                      const ULONG output_buffer_length)
	{
		...
		io_device_context<EmulatorTraits<Emu64>> context{c.emu};
		context.event = event;
	                                        ...
	                                        const emulator_object<ULONG> return_length)
	{
		...
		if (token_information_class == TokenUser)
		{
			...
			user.User.Sid = reinterpret_cast<unsigned long>(token_information + 0x10);
	                             ...
	                             ULONG ea_length)
	{
		...
		if (filename.starts_with(device_prefix))
		{
			...
			io_device_container<EmulatorTraits<Emu64>> container{std::move(device_name), c.win_emu, data};
	...
	NTSTATUS handle_NtRaiseException(const syscall_context& c,
	                                 const emulator_object<EXCEPTION_RECORD<EmulatorTraits<Emu64>>> /*exception_record*/,
	                                  ...
	                                  const ULONG initial_count, const ULONG maximum_count)
	{
		...
		if (object_attributes)
		{
			...
			if (attributes.ObjectName)
			{
				s.name = read_unicode_string(c.emu, reinterpret_cast<UNICODE_STRING<EmulatorTraits<Emu64>>*>(attributes.ObjectName));
			}
		}
	...
	NTSTATUS handle_NtAlertThreadByThreadIdEx(const syscall_context& c, const uint64_t thread_id,
	                                          const emulator_object<RTL_SRWLOCK<EmulatorTraits<Emu64>>> lock)
	{
		if (lock.value())
		{
